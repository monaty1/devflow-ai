...
export default function UuidGeneratorPage() {
  ...
  const VERSION_OPTIONS: { id: UuidVersion; label: string; description: string }[] = [
    { id: "v4", label: "v4", description: t("uuid.v4Desc") },
    { id: "v1", label: "v1", description: t("uuid.v1Desc") },
    { id: "v7", label: "v7", description: t("uuid.v7Desc") },
    { id: "nil", label: "Nil", description: t("uuid.nilDesc") },
    { id: "max", label: "Max", description: t("uuid.maxDesc") },
  ];

  const FORMAT_OPTIONS: { id: UuidFormat; label: string; example: string }[] = [
    { id: "standard", label: t("uuid.formatStandard"), example: "550e8400-e29b-..." },
    { id: "uppercase", label: t("uuid.formatUppercase"), example: "550E8400-E29B-..." },
    { id: "no-hyphens", label: t("uuid.formatNoHyphens"), example: "550e8400e29b..." },
    { id: "braces", label: t("uuid.formatBraces"), example: "{550e8400-...}" },
    { id: "urn", label: t("uuid.formatUrn"), example: "urn:uuid:550e..." },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {(
          [
            { id: "generate" as const, label: t("uuid.tabGenerate"), icon: Sparkles },
            { id: "validate" as const, label: t("uuid.tabValidate"), icon: Search },
            { id: "parse" as const, label: t("uuid.tabParse"), icon: Info },
          ]
        ).map((tab) => (
          <button
            key={tab.id}
      ...
      {activeTab === "generate" && (
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="space-y-4">
            {/* Version Selector */}
            <Card className="p-6">
              <h3 className="mb-4 font-semibold">{t("uuid.versionTitle")}</h3>
            ...
            {/* Format Selector */}
            <Card className="p-6">
              <h3 className="mb-4 font-semibold">{t("uuid.formatTitle")}</h3>
            ...
            {/* Quantity */}
            <Card className="p-6">
              <label htmlFor="uuid-quantity" className="mb-4 block font-semibold">{t("uuid.quantity")}</label>
              <input
                id="uuid-quantity"
                type="number"
              ...
              />
              <p className="mt-2 text-xs text-muted-foreground">{t("uuid.maxQuantity")}</p>
          ...
          <Card className="flex flex-col p-6">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold">{t("uuid.result")}</h2>
              ...
              <div className="flex-1 space-y-2 overflow-auto">
                <div className="mb-3 flex flex-wrap gap-2 text-xs">
                  ...
                  <span className="rounded-full bg-green-50 px-2 py-0.5 font-medium text-green-800 dark:bg-green-950 dark:text-green-300">
                    {result.uuids.length > 1 ? t("uuid.uuidCountPlural", { count: result.uuids.length }) : t("uuid.uuidCount", { count: result.uuids.length })}
                  </span>
                </div>
      ...
      {activeTab === "validate" && (
        <Card className="p-6">
          <label htmlFor="uuid-validate" className="mb-4 block text-lg font-semibold">{t("uuid.validateTitle")}</label>
          <input
            id="uuid-validate"
            type="text"
            value={validateInput}
            ...
            <div className="mt-4 space-y-3">
              ...
              >
                ...
                <span className="font-medium">
                  {validationResult.isValid ? t("uuid.validUuid") : validationResult.error ?? t("uuid.invalidUuid")}
                </span>
              </div>
              ...
              {validationResult.isValid && (
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-lg bg-muted/50 px-4 py-3">
                    <span className="text-xs text-muted-foreground">{t("uuid.versionLabel")}</span>
                    <p className="font-mono font-bold">{validationResult.version ?? "unknown"}</p>
                  </div>
                  <div className="rounded-lg bg-muted/50 px-4 py-3">
                    <span className="text-xs text-muted-foreground">{t("uuid.variantLabel")}</span>
      ...
      {activeTab === "parse" && (
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-lg font-semibold">{t("uuid.parseTitle")}</h2>
          ...
          {parsedInfo && (
            <div className="mt-4 space-y-3">
              ...
              >
                ...
                <span className="font-medium">
                  {parsedInfo.isValid ? t("uuid.validUuid") : t("uuid.invalidUuid")}
                </span>
              </div>
              ...
              {parsedInfo.isValid && (
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-lg bg-muted/50 px-4 py-3">
                    <span className="text-xs text-muted-foreground">{t("uuid.normalizedUuid")}</span>
                  ...
                  {parsedInfo.timestamp !== undefined && (
                    <div className="rounded-lg bg-muted/50 px-4 py-3">
                      <span className="text-xs text-muted-foreground">{t("uuid.timestamp")}</span>
                  ...
                  {parsedInfo.clockSeq !== undefined && (
                    <div className="rounded-lg bg-muted/50 px-4 py-3">
                      <span className="text-xs text-muted-foreground">{t("uuid.clockSequence")}</span>
                      <p className="font-mono font-bold">{parsedInfo.clockSeq}</p>
                    </div>
                  )}
                  {parsedInfo.node !== undefined && (
                    <div className="rounded-lg bg-muted/50 px-4 py-3">
                      <span className="text-xs text-muted-foreground">{t("uuid.nodeMac")}</span>
